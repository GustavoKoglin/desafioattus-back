...
if (!fs_1.default.existsSync(DB_FILE)) {
    const adminPassword = bcryptjs_1.default.hashSync(process.env.ADMIN_PASSWORD || 'DemoAdminPass123!', 10);
    ...
    fs_1.default.writeFileSync(DB_FILE, JSON.stringify(initialData, null, 2));
}
// Utilitários de leitura/escrita
const readDB = () => JSON.parse(fs_1.default.readFileSync(DB_FILE, 'utf-8'));
const writeDB = (data) => fs_1.default.writeFileSync(DB_FILE, JSON.stringify(data, null, 2));
// Adicionar Log
const addLog = (action, authorName, authorEmail, ip) => {
    const db = readDB();
    const now = new Date();
    const log = {
        id: (0, uuid_1.v4)(),
...
app.post('/api/login', (req, res) => {
    ...
    const token = jsonwebtoken_1.default.sign({ id: user.id, email: user.email, role: user.role, name: user.name }, SECRET_KEY, { expiresIn: '8h' });
...
app.get('/api/users', authMiddleware, (req, res) => {
    ...
    const safeUsers = db.users
        .filter((u) => !u.role && u.type !== 'Platform') // Filtra quem tem cargo ou é da plataforma
        .map((u) => {
...
app.post('/api/users', authMiddleware, roleMiddleware(['Admin', 'Editor']), (req, res) => {
    const db = readDB();
    ...
    const newUser = {
        ...
        password: password ? bcryptjs_1.default.hashSync(password, 10) : ''
    };
...
app.put('/api/users/:id', authMiddleware, roleMiddleware(['Admin', 'Editor']), (req, res) => {
    const db = readDB();
    ...
    db.users[index] = {
        ...
        password: password ? bcryptjs_1.default.hashSync(password, 10) : oldUser.password
    };
    writeDB(db);
...
app.delete('/api/users/:id', authMiddleware, roleMiddleware(['Admin', 'Editor']), (req, res) => {
    const db = readDB();
    const index = db.users.findIndex((u) => u.id === req.params.id);
    if (index === -1) {
        return res.status(404).json({ message: 'Usuário não encontrado' });
...
app.get('/api/logs', authMiddleware, roleMiddleware(['Admin']), (req, res) => {
    const db = readDB();
    res.json(db.logs);
});
// --- PLATFORM USERS ENDPOINTS ---
app.get('/api/platform-users', authMiddleware, roleMiddleware(['Admin', 'Editor']), (req, res) => {
    const db = readDB();
    const platformUsers = db.users
        .filter((u) => u.type === 'Platform' || u.role === 'Admin')
        .map((u) => {
        const { password, ...rest } = u;
        return rest;
    });
...
app.post('/api/platform-users', authMiddleware, roleMiddleware(['Admin']), (req, res) => {
    const db = readDB();
...
app.put('/api/platform-users/:id', authMiddleware, roleMiddleware(['Admin']), (req, res) => {
    const db = readDB();
    const index = db.users.findIndex((u) => u.id === req.params.id);
    if (index === -1)
        return res.status(404).json({ message: 'Usuário não encontrado' });
...
app.delete('/api/platform-users/:id', authMiddleware, roleMiddleware(['Admin']), (req, res) => {
    const db = readDB();
    const index = db.users.findIndex((u) => u.id === req.params.id);
    if (index === -1)
        return res.status(404).json({ message: 'Usuário não encontrado' });
